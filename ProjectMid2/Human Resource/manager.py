...
def add_staff(id, name, gender, DoB, password, frame, window_m):                        # Add Staff info to s_list
    try:
        f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staffs.pickle", "rb")    # Read Staffs.pickle
        ...
        if ckF.prevent_duplicate2(office, s_list) == False:
            ...
            if ckF.check_gender(gender.get()) == False:
                ...
                if len(s_list) > 7 :
                    ...
                    f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staffs.pickle", "wb")        # Create new Staffs.pickle
                    pickle.dump(s_list, f)                                                              # Dump s_list to Staffs.pickle
                    f.close()
...
def update_one_staff_salary(id, working_hour, wage, frame, window_m):           # Update salary of one Staff in s_list
    try:
        ...
        m_list = []
    f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staffs.pickle", "rb")                # Read database file
    s_list = pickle.load(f)
    f.close()
...
def show_all(window_m):     # Show all employee info and salary
    ...
    f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staffs.pickle", "rb")        # Read file
    s_list = pickle.load(f)
    f.close()
    ...
    frame_label5.grid(row = 0, column = 4)
    try:
        f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Employees.pickle", "rb") # Read the file
        all_e = pickle.load(f)
        f.close()
