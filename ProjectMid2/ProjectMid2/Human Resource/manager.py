...
def show_staff(frame, window_m):                # Show s_list
    ...
    try:
        for i in range(ord('A'), ord('Z') + 1):     # i run from A to Z
            try:
                f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staff{}.pickle".format(chr(i)), "rb")
                staff = pickle.load(f)
                f.close()
                s_list.append(staff)
            except:
...
def add_staff(id, name, gender, DoB, password, frame, window_m):                        # Add Staff info to s_list
    try:
        f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staff_s.pickle", "rb")    # Read Staff_s.pickle
        s_list = pickle.load(f)
        f.close()
    except:
        s_list =[]                                                                      # Create new list if no data
    if ckF.prevent_duplicate(id.get(), s_list) == False:
        ...
        if ckF.prevent_duplicate2(office, s_list) == False:
            ...
            if ckF.check_gender(gender.get()) == False:
                ...
                if len(s_list) > 7 :
                    ...
                    f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staff_s.pickle", "wb")        # Create new Staff_s.pickle
                    ...
                    for i in range(len(s_list)):
                        ...
                        pickle.dump(s_list[i], f)                                                       # Divide Staff_s in s_list base on staff.office
...
def del_staff(del_id, frame, window_m):                                                 # Del Staff info in s_list
    ...
    if not any(staff.id == del_id.get() for staff in s_list):
        ...
        pickle.dump(s_list, f)                                                          # Dump s_list to Staff_s.pickle
        f.close()
        os.remove("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staff{}.pickle".format(del_id.get()[0]))    # Delete database file of that Staff
        del_id.delete(0, END)
        for i in range(len(s_list)):
            f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staff{}.pickle".format(s_list[i].office), "wb")
...
def show_all_staff_salary(frame, window_m):         # Show m_list 
    ...
    frame_label6.grid(row = 0, column = 5)
    try:
        f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staff Salary.pickle", "rb")      # Read Staff Salary
...
def update_one_staff_salary(id, working_hour, wage, frame, window_m):           # Update salary of one Staff in s_list
    try:
        f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staff Salary.pickle", "rb")      # Read database file
        m_list = pickle.load(f)
        f.close()
    except:
        m_list = []
    f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staff_s.pickle", "rb")                # Read database file
    s_list = pickle.load(f)
    f.close()
...
def show_all(window_m):     # Show all employee info and salary
    ...
    temp.title("Employee_s INFO - SALARY")
    ...
    f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Staff_s.pickle", "rb")        # Read file
    s_list = pickle.load(f)
    f.close()
    ...
    for i in range(len(s_list)):
        try:
            ...
            continue
    f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Employee_s.pickle", "wb")
    ...
    frame_label5.grid(row = 0, column = 4)
    try:
        f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Employee_s.pickle", "rb") # Read the file
        all_e = pickle.load(f)
        f.close()
    ...
    frame_label6.grid(row = 0, column = 5)
    try:
        f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Salaries.pickle", "rb")  # Read the file
        all_p = pickle.load(f)
        f.close()
...
def update_manager_info(manager_label, window_m):       # Update manager info
    ...
    def update(name, gender, DoB, password, window_m, manager_label, temp):
        f = open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Database\\Manager.pickle", "rb")          # Read file
...
""".format(id = manager.id, name = manager.name, gender = manager.gender, DoB = manager.DoB, password = manager.password))
    ...
    icon3 = ImageTk.PhotoImage(Image.open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\dreamcatcher.png"))
...
def main():
    ...
    icon1 = ImageTk.PhotoImage(Image.open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\dreamcatcher.png"))     
    label = Label(window_m, image = icon1)
    label.grid(row = 0, column = 0)     # Display image
    
    icon2 = ImageTk.PhotoImage(Image.open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\dreamcatcher.png"))
    label = Label(window_m, image = icon2)
    label.grid(row = 0, column = 2)     # Display image
    
    icon3 = ImageTk.PhotoImage(Image.open("C:\\Users\\FLOS IGNIS\\OneDrive\\Máy tính\\Human Resource\\Manager.png"))
